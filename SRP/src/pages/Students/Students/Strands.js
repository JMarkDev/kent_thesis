        ...
        <div className='flex flex-wrap justify-center items-center mt-10'>
        {
          strand.map((strand) => (
            // <div className='m-5 dark:bg-[#273242]'>
              <div key={strand.id}  className='my-5 dark:bg-[#273242] shadow-lg bg-white rounded p-5 flex flex-col lg:flex-row items-center'>
                <img className='w-full lg:w-[50%] h-96 rounded-lg' src={`http://localhost:3001/uploads/${strand.image.split(',')[0]}`} alt='strand img' />
                <div className='w-full lg:w-[50%]'>
                  <h1 className='text-2xl lg:ml-5 my-3 leading-normal'>{strand.name}</h1>
                  <p className='lg:ml-5 leading-normal pb-10'>{truncateText(strand.description, 500)}</p>
